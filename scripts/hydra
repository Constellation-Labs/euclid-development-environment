...
check_if_framework_name_is_set() {
  if [[ -z "$FRAMEWORK_NAME" ]]; then
    echo "You should provide the FRAMEWORK_NAME on .env file"
    exit
  fi
}

check_if_framework_directory_exists() {
  cd source/
  if [ ! -d "framework/$FRAMEWORK_NAME" ]; then
...
install() {
  ...
  create_template framework $FRAMEWORK_NAME
...
build() {
  ...
  if [[ ! -z "$argc_include_dag_l1" || "$argc_only" == "dag-l1" ]]; then
    ...
    if [ ! -z "$argc_run" ]; then
      ...
      join_l1_global_nodes
      dag_l1_1_url="DAG L1 - 1: http://localhost:9500/cluster/info"
      dag_l1_2_url="DAG L1 - 2: http://localhost:9600/cluster/info"
      dag_l1_3_url="DAG L1 - 3: http://localhost:9700/cluster/info"
    fi
    cd ../
  fi
  ...
  if [[ -z "$argc_only" || "$argc_only" == "currency-l0" ]]; then
    ...
    if [ ! -z "$argc_no_cache" ]; then
      ...
      docker-compose build --build-arg TEMPLATE_NAME=$FRAMEWORK_NAME --build-arg GIT_PERSONAL_ACCESS_TOKEN=$GITHUB_TOKEN --build-arg P12_FILE_NAME=$P12_GENESIS_FILE_NAME --build-arg P12_FILE_KEY_ALIAS=$P12_GENESIS_FILE_KEY_ALIAS --build-arg P12_FILE_PASSWORD=$P12_GENESIS_FILE_PASSWORD --no-cache
    else
  ...
  if [[ -z "$argc_only" || "$argc_only" == "currency-l1" ]]; then
    ...
    if [ ! -z "$argc_no_cache" ]; then
      ...
      docker-compose build --build-arg TEMPLATE_NAME=$FRAMEWORK_NAME --build-arg GIT_PERSONAL_ACCESS_TOKEN=$GITHUB_TOKEN --build-arg P12_FILE_NAME_GENESIS=$P12_GENESIS_FILE_NAME --build-arg P12_FILE_NAME_NODE_2=$P12_NODE_2_FILE_NAME --build-arg P12_FILE_NAME_NODE_3=$P12_NODE_3_FILE_NAME --build-arg P12_FILE_KEY_ALIAS_GENESIS=$P12_GENESIS_FILE_KEY_ALIAS --build-arg P12_FILE_KEY_ALIAS_NODE_2=$P12_NODE_2_FILE_KEY_ALIAS --build-arg P12_FILE_KEY_ALIAS_NODE_3=$P12_NODE_3_FILE_KEY_ALIAS --build-arg P12_FILE_PASSWORD_GENESIS=$P12_GENESIS_FILE_PASSWORD --build-arg P12_FILE_PASSWORD_NODE_2=$P12_NODE_2_FILE_PASSWORD --build-arg P12_FILE_PASSWORD_NODE_3=$P12_NODE_3_FILE_PASSWORD --no-cache
    else
...
# @flag  --delete_framework          Delete framework
# @option --only                     Build specific layer. Options: global-l0, dag-l1, currency-l0, currency-l1, monitoring
destroy() {
  check_if_docker_is_running
  BASEDIR=$(dirname "$0")
  ...
  if [[ ! -z "$argc_delete_framework" ]]; then
    echo "Removing the template codebase at $1..."
    rm -r source/framework/$FRAMEWORK_NAME
    echo "Removed!"
  fi

...
import { RemoteStartError } from '../core/errors.js';
import { logger } from '../core/logger.js';
import { fetchNodeInfo, waitForNodeReady } from '../core/cluster/health.js';
import type { EuclidConfig, RemoteHostConfig, DeployConfig } from '../core/config/schema.js';
...
export async function remoteStart(options: RemoteStartOptions): Promise<void> {
  ...
  if (!deploy) {
    throw new RemoteStartError('(config)', 'metagraph', { cause: new Error('No deploy configuration found') });
  }
  ...
  try {
    ...
    const metagraphIdResult = await ssh.exec(firstHost, `cat "${ml0Dir}/genesis.address" 2>/dev/null || echo ""`);
...
async function startLayer(
  ...
  const codeDir = remoteCodeDir(firstHost, layer);
...
async function cleanupAllHosts(
  ...
  const allPorts = [
    remotePorts.metagraph_l0.public, remotePorts.metagraph_l0.p2p, remotePorts.metagraph_l0.cli,
    remotePorts.currency_l1.public, remotePorts.currency_l1.p2p, remotePorts.currency_l1.cli,
    remotePorts.data_l1.public, remotePorts.data_l1.p2p, remotePorts.data_l1.cli,
  ];
...
async function waitForState(
  ...
  throw new Error(`Node at ${host}:${port} did not reach ${targetState} after ${maxRetries} attempts`);
}

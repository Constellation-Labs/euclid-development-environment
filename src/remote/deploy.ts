...
import { RemoteDeployError } from '../core/errors.js';
import { logger } from '../core/logger.js';
import type { EuclidConfig, RemoteHostConfig } from '../core/config/schema.js';
  ...
  for (const jar of requiredJars) {
    if (!existsSync(resolve(jarsDir, jar))) {
      throw new RemoteDeployError(
        '(local)',
        `Missing JAR: ${jar}. Run 'hydra build' first.`,
      );
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < deploy.hosts.length; i++) {
      ...
      if (config.snapshot_fees) {
        ...
        const stakingP12 = resolve(dataPath, 'p12-files', config.snapshot_fees.staking.key_file.name);
        if (existsSync(stakingP12)) {
          await ssh.upload(host, stakingP12, `${ml0Dir}/${config.snapshot_fees.staking.key_file.name}`);
        }
      }
...
async function hasExistingData(ssh: SSHManager, host: RemoteHostConfig): Promise<boolean> {
  try {
    const result = await ssh.exec(host, `test -d "${remoteCodeDir(host, 'metagraph-l0')}/data/incremental_snapshot" && echo "yes" || echo "no"`);

...
import type { EuclidConfig, DeployConfig, GenesisMeta } from '../../../index.js';
import {
  formatError,
  formatSuccess,
  formatHeader,
...
export async function createRemoteGenesisCommand(options: { verbose?: boolean }): Promise<void> {
  ...
  try {
    ...
    // Confirm before overwriting an existing remote genesis — the genesis files
    // define the metagraph ID and cannot be regenerated byte-identically (random
    // signature nonce + "latest global snapshot" anchor), so overwriting a
    // whitelisted genesis is unrecoverable.
    if (existsSync(resolve(genesisDir, 'genesis.snapshot'))) {
      const prevAddressPath = resolve(genesisDir, 'genesis.address');
      const prevId = existsSync(prevAddressPath)
        ? (await readFile(prevAddressPath, 'utf-8')).trim()
        : '';
      process.stdout.write(
        `\n  ${t.warn('⚠')}  A remote genesis already exists${prevId ? ` (${t.white(prevId)})` : ''}.\n` +
          `     Overwriting it CANNOT be undone and the new genesis will have a\n` +
          `     ${t.white('different metagraph ID')} — if the current ID is whitelisted on the\n` +
          `     network, a new whitelisting round will be needed.\n\n`,
      );
      const overwrite = await confirm({
        message: 'Overwrite the existing remote genesis?',
        default: false,
      });
      if (!overwrite) {
        process.stdout.write(`\n  ${t.dim('Cancelled — existing genesis left untouched.')}\n\n`);
        return;
      }
    }
    ...
    if (hasSnapshot && hasAddress) {
      ...
      // Write metadata next to the genesis for traceability and deploy-time validation
      const meta: GenesisMeta = {
        metagraphId,
        network: net.name,
        gl0NodeIp: net.gl0_node.ip,
        gl0NodePort: Number(net.gl0_node.public_port),
        createdAt: new Date().toISOString(),
      };
      await writeFile(resolve(genesisDir, GENESIS_META_FILE), JSON.stringify(meta, null, 2) + '\n');
      ...
      process.stdout.write(`    ${t.dim('docker/artifacts/genesis/')}${GENESIS_META_FILE}\n`);
      if (metagraphId) {
        process.stdout.write(`\n  ${formatKeyValue('Metagraph ID', metagraphId)}`);
      }
      process.stdout.write(
        `\n  ${t.warn('⚠')}  Back these files up (e.g. commit them to git) — they ${t.white('are')} your\n` +
          `     metagraph identity and cannot be regenerated with the same ID.\n` +
          `     Note: local runs never touch them (local genesis lives in ${t.dim('genesis-local/')}).\n`,
      );

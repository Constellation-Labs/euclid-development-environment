  ...
  try {
    ...
    for (const host of config.deploy.hosts) {
      ...
      if (config.layers.includes('metagraph-l0')) {
        process.stdout.write(
          `      Metagraph L0    http://${host.host}:${remotePorts.metagraph_l0.public}\n`,
        );
      }
      if (config.layers.includes('currency-l1')) {
        process.stdout.write(
          `      Currency L1     http://${host.host}:${remotePorts.currency_l1.public}\n`,
        );
      }
      if (config.layers.includes('data-l1')) {
        process.stdout.write(
          `      Data L1         http://${host.host}:${remotePorts.data_l1.public}\n`,
        );
      }
    }
    process.stdout.write('\n');

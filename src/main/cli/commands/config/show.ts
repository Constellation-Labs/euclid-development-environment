...
export async function configShowCommand(options: {
  ...
  try {
    ...
    if (options.json) {
      ...
      process.stdout.write(
        `  Tessellation:     ${config.tessellation_version} (${config.tessellation_ref_type})\n`,
      );
      process.stdout.write(
        `  Framework:        ${config.framework.name} v${config.framework.version}\n`,
      );
      process.stdout.write(
        `  Modules:          ${config.framework.modules.length > 0 ? config.framework.modules.join(', ') : 'none'}\n`,
      );
      ...
      process.stdout.write(
        `  Grafana:          ${config.docker.start_grafana_container ? 'enabled' : 'disabled'}\n`,
      );
      if (config.deploy) {

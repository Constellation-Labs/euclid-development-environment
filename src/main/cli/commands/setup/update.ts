...
export async function updateCommand(options: {
  ...
  try {
    ...
    process.stdout.write('    - src/ (CLI source code + tests)\n');
    ...
    try {
      ...
      const hydraContainers = containers.filter((c) =>
        c.Names.some(
          (n) =>
            n.includes('metagraph') ||
            n.includes('global') ||
            n.includes('dag') ||
            n.includes('grafana') ||
    ...
    if (actualSrcDocker) {
      ...
      for (const sub of subDirs) {
        const primaryPath = resolve(actualSrcDocker, sub);
        const srcSub = existsSync(primaryPath)
          ? primaryPath
          : resolve(actualSrcDocker, 'docker', sub);
    ...
    // Update config files (package.json, tsconfigs, linter, formatter, test runner)

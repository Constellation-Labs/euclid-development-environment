...
export interface RemoteStartOptions {
  ...
  genesis?: boolean;
  /**
   * Rollback-only: re-send owner + staking signing messages with a background
   * spammer loop to defend against the mainnet consensus-round-2 race window.
   * Ignored when `genesis === true`.
   */
  ...
  try {
    ...
    if (config.layers.includes('metagraph-l0')) {
      ...
      try {
        ...
        // Always stop spammers — on Ready (success) or on failure path (so
        // background POST loop doesn't keep firing while user debugs).

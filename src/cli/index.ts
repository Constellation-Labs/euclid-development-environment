...
import { logger, LogLevel } from '../core/index.js';
...
import { installTemplateCommand } from './commands/install-template.js';
import { installMonitoringServiceCommand } from './commands/install-monitoring-service.js';
import { createRemoteGenesisCommand } from './commands/create-remote-genesis.js';
import { updateCommand } from './commands/update.js';
import { checkSeedlistCommand } from './commands/check-seedlist.js';
...
const configCmd = program
  .command('config')
  .description('Manage euclid.json configuration');
...
program
  ...
  .action(async () => {
    process.stderr.write("Warning: 'start-genesis' is deprecated. Use 'hydra start --genesis' instead.\n\n");
    const opts = program.opts();
...
const remoteCmd = program
  .command('remote')
  .description('Manage remote metagraph deployment');
...
remoteCmd
  ...
  .action(async (cmdOpts) => {
    const opts = program.opts();
    await remoteStartMonitoringCommand({ forceRestart: cmdOpts.forceRestart, verbose: opts.verbose });
  });

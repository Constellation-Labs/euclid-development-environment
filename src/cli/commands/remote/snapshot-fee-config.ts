...
export async function remoteSnapshotFeeConfigCommand(options: {
  ...
  try {
    ...
    try {
      ...
      const currencySnapshots = (data[1] as Record<string, unknown>)?.lastCurrencySnapshots as Record<string, unknown> | undefined;
      const metagraphSnapshot = currencySnapshots?.[metagraphId] as { Right?: unknown[] } | undefined;
      const lastMessages = ((metagraphSnapshot?.Right?.[1]) as Record<string, unknown>)?.lastMessages as Record<string, unknown> | undefined;

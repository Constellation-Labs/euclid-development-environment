...
export class ConfigValidationError extends ConfigError {
  ...
  constructor(issues: Array<{ path: string; message: string }>) {
    ...
    super(summary, {
      suggestion: "Run 'hydra config validate' to see all issues, or 'hydra config show' to inspect your config.",
    });
...
export class DockerVersionError extends DockerError {
  constructor(found: string, required: string) {
    super(`Docker version ${found} is too old (required: >= ${required})`, {
      suggestion: 'Update Docker Desktop to the latest version: https://docs.docker.com/get-docker/',
    });
